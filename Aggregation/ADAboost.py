...
rmse = np.sqrt(mse_ada)  # 计算均方根误差（RMSE）
r2_ada = r2_score(y_test, y_pred_ada)

# 定义贝叶斯优化的目标函数


def optimize_ada(buffer_te, buffer_tris_hcl, buffer_hepes, buffer_pbs, aptamer, mba, time):
    # 构造输入特征
    ...
    # 预测CAP的萃取量值
    extraction_pred = ada_regressor.predict(input_features_scaled)
...
optimizer = BayesianOptimization(
    f=optimize_ada, pbounds=pbounds, random_state=42)
...
# 预测最优条件下的CAP的萃取量值
optimal_extraction = ada_regressor.predict(optimal_input_scaled)[0]
...
print("\nModel Evaluation on Test Set (AdaBoost):")
print(f"Mean Squared Error (MSE): {mse_ada:.4f}")
print(f"Root Mean Squared Error (RMSE): {rmse:.4f}")
print(f"R-squared (R²): {r2_ada:.4f}")
