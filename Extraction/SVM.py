...
svr_regressor = SVR(
    kernel='rbf',  # 使用 RBF 核
    C=10,          # 正则化参数
    epsilon=0.1,   # ε-Insensitive 损失函数的参数
    gamma='scale'  # 核函数的系数
)
...
def optimize_svr(buffer_te, buffer_tris_hcl, buffer_hepes, buffer_pbs,
                 ...
                 mg_concentration, extraction_time, desorption_time, C, epsilon):
    # 构造输入特征
    ...
    # 预测CAP的萃取量值
    extraction_pred = svr_model.predict(input_features_scaled)
...
optimizer = BayesianOptimization(
    f=optimize_svr, pbounds=pbounds, random_state=42, verbose=2
)
...
optimal_svr = SVR(
    kernel='rbf',
    C=optimal_conditions['C'],
    epsilon=optimal_conditions['epsilon'],
    gamma='scale'
)
optimal_svr.fit(X_train_scaled, y_train)

# 预测最优条件下的CAP的萃取量值
optimal_extraction = optimal_svr.predict(optimal_input_scaled)
