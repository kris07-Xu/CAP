...
import pandas as pd
import numpy as np
from sklearn.ensemble import RandomForestRegressor  # 替换为随机森林
...
# 初始化随机森林回归模型
rf_regressor = RandomForestRegressor(random_state=42)  # 使用随机森林
...
rmse = np.sqrt(mse_rf)  # 计算均方根误差（RMSE）
r2_rf = r2_score(y_test, y_pred_rf)

# 定义贝叶斯优化的目标函数


def optimize_rf(buffer_te, buffer_tris_hcl, buffer_hepes, buffer_pbs, aptamer, mba, time):
    # 构造输入特征
    ...
    # 预测CAP的萃取量值
    extraction_pred = rf_regressor.predict(input_features_scaled)
...
optimizer = BayesianOptimization(
    f=optimize_rf, pbounds=pbounds, random_state=42)
...
# 预测最优条件下的CAP的萃取量值
optimal_extraction = rf_regressor.predict(optimal_input_scaled)[0]
