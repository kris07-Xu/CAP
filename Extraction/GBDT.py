...
# 加载数据
file_path = '######.xlsx'  # 文件路径
data = pd.read_excel(file_path)  # 打印数据的列名
...
# 对分类变量进行 One-Hot 编码
data_encoded = pd.get_dummies(data, columns=['不同萃取缓冲液', '解析液'], prefix=['Buffer', 'Solvent'])
...
# 定义贝叶斯优化的目标函数
def optimize_gbdt(buffer_te, buffer_tris_hcl, buffer_hepes, buffer_pbs,
    ...
    # 预测CAP的萃取量值
    extraction_pred = gbdt_regressor.predict(input_features)
...
# 预测最优条件下的CAP的萃取量值
optimal_extraction = gbdt_regressor.predict(optimal_input)
