...
import pandas as pd
import numpy as np
from sklearn.ensemble import GradientBoostingRegressor  # GBDT回归模型
...
# 初始化GBDT回归模型
gbdt_regressor = GradientBoostingRegressor(random_state=42, n_estimators=100, learning_rate=0.1, max_depth=3)

# 训练GBDT模型
gbdt_regressor.fit(X_train_scaled, y_train)
print("\nGradient Boosting Decision Tree (GBDT) Model training completed.")

# 在测试集上评估GBDT模型
y_pred_gbdt = gbdt_regressor.predict(X_test_scaled)
mse_gbdt = mean_squared_error(y_test, y_pred_gbdt)
rmse = np.sqrt(mse_gbdt)  # 计算均方根误差（RMSE）
r2_gbdt = r2_score(y_test, y_pred_gbdt)

# 定义贝叶斯优化的目标函数
def optimize_gbdt(buffer_te, buffer_tris_hcl, buffer_hepes, buffer_pbs, aptamer, mba, time):
    # 构造输入特征
    ...
    # 预测CAP的萃取量值
    extraction_pred = gbdt_regressor.predict(input_features_scaled)
...
optimizer = BayesianOptimization(
    f=optimize_gbdt, pbounds=pbounds, random_state=42)
...
# 使用GBDT模型进行预测
optimal_extraction = gbdt_regressor.predict(optimal_input_scaled)[0]
...
print("\nModel Evaluation on Test Set (GBDT):")
print(f"Mean Squared Error (MSE): {mse_gbdt:.4f}")
print(f"Root Mean Squared Error (RMSE): {rmse:.4f}")
print(f"R-squared (R²): {r2_gbdt:.4f}")
