...
cart_regressor = DecisionTreeRegressor(
    criterion='friedman_mse',  # 使用弗里德曼改进的均方误差
    max_depth=5,              # 树的最大深度，可以根据需要调整
...
print("\nCART Model training completed.")

# 在测试集上评估决策树模型
y_pred_cart = cart_regressor.predict(X_test_scaled)
mse_cart = mean_squared_error(y_test, y_pred_cart)
rmse = np.sqrt(mse_cart)  # 计算均方根误差（RMSE）
r2_cart = r2_score(y_test, y_pred_cart)

# 定义贝叶斯优化的目标函数


def optimize_cart(buffer_te, buffer_tris_hcl, buffer_hepes, buffer_pbs, aptamer, mba, time):
    # 构造输入特征
    ...
    # 预测CAP的萃取量值
    extraction_pred = cart_regressor.predict(input_features_scaled)
...
optimizer = BayesianOptimization(
    f=optimize_cart, pbounds=pbounds, random_state=42
)
...
# 预测最优条件下的CAP的萃取量值
optimal_extraction = cart_regressor.predict(optimal_input_scaled)[0]
...
print(f"Mean Squared Error (MSE): {mse_cart:.4f}")
print(f"Root Mean Squared Error (RMSE): {rmse:.4f}")
print(f"R-squared (R²): {r2_cart:.4f}")
